...
import { IEAA_ITEMS, IEAA_DIMENSIONS_INFO } from '@/src/core/content/ieaa-items';
import { QuestionCard } from '@/components/questionnaire/QuestionCard';
import { saveIEAAAnswers } from '@/app/actions/assessment';
import { ChevronLeft, ChevronRight, CheckCircle2, Loader2, BookOpen, AlertCircle, Maximize2, Minimize2 } from 'lucide-react';
import { IEAARawAnswers, IEAADimension } from '@/src/core/types';
import { cn } from '@/lib/utils';
import { toast } from 'sonner';

const DIMENSIONS = Object.values(IEAADimension);

const DIMENSION_COLORS: Record<IEAADimension, any> = {
    [IEAADimension.COGNITIVA]: { from: 'from-blue-500', to: 'to-indigo-600', bg: 'bg-blue-50', text: 'text-blue-600', ring: 'ring-blue-200' },
    [IEAADimension.METACOGNITIVA]: { from: 'from-purple-500', to: 'to-violet-600', bg: 'bg-purple-50', text: 'text-purple-600', ring: 'ring-purple-200' },
    [IEAADimension.GESTAO_RECURSOS]: { from: 'from-emerald-500', to: 'to-teal-500', bg: 'bg-emerald-50', text: 'text-emerald-600', ring: 'ring-emerald-200' },
    [IEAADimension.MOTIVACIONAL]: { from: 'from-amber-500', to: 'to-orange-500', bg: 'bg-amber-50', text: 'text-amber-600', ring: 'ring-amber-200' },
};

export function IEAAWizard({
    initialAnswers = {},
    studentId,
    studentName = 'Aluno',
}: {
    ...
    const [answers, setAnswers] = useState<IEAARawAnswers>(initialAnswers);
    ...
    const currentDimension = DIMENSIONS[currentStepIdx];
    const stepConfig = IEAA_DIMENSIONS_INFO[currentDimension];
    const isLastStep = currentStepIdx === DIMENSIONS.length - 1;
    const colors = DIMENSION_COLORS[currentDimension];
    ...
    const handleAnswerChange = (questionId: number, value: number) => {
        ...
        startTransition(async () => {
            try {
                await saveIEAAAnswers(newAnswers, studentId);
            } catch (err) {
    ...
    const handleNext = async () => {
        ...
        if (isLastStep) {
            ...
            try {
                const result = await saveIEAAAnswers(answers, studentId);
    ...
    return (
        <div className={cn(
                ...
                <header className="fixed top-0 left-0 right-0 h-20 bg-white/90 backdrop-blur-xl border-b border-slate-200/60 z-50 px-6 sm:px-10 flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-3.5">
                        ...
                        <div className="hidden sm:block">
                            ...
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">IEAA</span>
                        </div>
                    </div>
            ...
            <div className={cn(
                ...
                {!isFocusMode && (
                    <div className="flex justify-end mb-4">
                        <button
                            ...
                            className="flex items-center gap-2 text-xs font-bold text-emerald-600 bg-emerald-50 hover:bg-emerald-100 px-4 py-2 rounded-xl transition-colors"
                        >
                            <Maximize2 size={14} />
                            Modo Foco
                        </button>
                ...
                <div className={cn(
                    ...
                    <div className="flex items-center gap-2 px-1">
                        {DIMENSIONS.map((_, idx) => {
                            const dimConfig = IEAA_DIMENSIONS_INFO[DIMENSIONS[idx]];
                            const stepItems = dimConfig.items;
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        isActive
                                            ? `bg-gradient-to-r ${DIMENSION_COLORS[DIMENSIONS[idx]].from} ${DIMENSION_COLORS[DIMENSIONS[idx]].to} shadow-lg scale-y-125`
                                            : stepAllAnswered
                                                ? 'bg-emerald-400'
                                                : idx < currentStepIdx
                                                    ? 'bg-slate-300'
                    ...
                    <div className="flex items-end justify-between px-1">
                        <div className="flex items-center gap-3">
                            <div className={cn(
                                "h-10 w-10 rounded-2xl bg-gradient-to-br flex items-center justify-center text-white text-base font-black shadow-lg shadow-emerald-100",
                ...
                <div key={currentDimension} className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
                    {stepConfig.items.map((id, idx) => {
                        const item = IEAA_ITEMS.find(i => i.id === id);
                        if (!item) return null;
                        return (
                            <div
                                key={id}
                            ...
                            className={cn(
                                ...
                                !allCurrentAnswered
                                    ...
                                    : isLastStep
                                        ...
                                        : `bg-gradient-to-r ${colors.from} ${colors.to} text-white shadow-emerald-200`
                            )}
                        >
                            {isSaving ? (
